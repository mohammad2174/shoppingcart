  ...
  setSelectedSize = () => {
    ...
    render() {
      ...
      function classNames(...classes) {
        ...
        const belts = products.slice(48, 52)
        const product = {
            ...
            {belts.map((belt) => (
              <div key={belt.id} className="group relative">
                ...
                <div className="mt-4 flex justify-between">
                  ...
                  <p className="mt-1 text-sm text-gray-500">X{belt.inventory ? belt.inventory : <span className="text-red-600">Has Ended</span>}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  className={this.state.belt.inventory ? "w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm" : "w-full inline-flex cursor-not-allowed justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm"}
                  onClick={() => addToCard(this.state.belt.id) && this.setmodalClose()}
                  disabled = {this.state.belt.inventory ? '' : 'disabled'}
                >
                  {this.state.belt.inventory > 0 ? 'Activate page' : 'Sold Out'}
                </button>
                <button
                  type="button"
