...
class Bags extends Component {
  ...
  setOpen = (bag) => {
    this.setState({open : true, bag : bag})
  }
    ...
    render() {
      ...
      return (
        ...
        <div className="flex gap-2 text-base font-extrabold tracking-tight text-gray-900 sm:gap-5">
          ...
          <Link to="/bags" className="text-gray-400">Bags</Link>
          </div>
          <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {bags.map((bag) => (
              <div key={bag.id} className="group relative">
                ...
                <button onClick={() => this.setOpen(bag)} class="relative z-10 w-full bg-white bg-opacity-75 py-2 px-4 rounded-md text-sm text-gray-900 opacity-0 group-hover:opacity-100 focus:opacity-100">Quick View</button>
                </div>  
                </div>
                    ...
                    <section aria-labelledby="options-heading" className="mt-10">
                      <h3 id="options-heading" className="sr-only">
                        ...
                        <button
                          ...
                          onClick={() => addToCard(this.state.bag.id, this.state.selectedColor, this.state.selectedSize) && this.setClose() || this.setNotifyOpen() || this.setTimeout()}
                        >
                          Add to bag
                        </button>
